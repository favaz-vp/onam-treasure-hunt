...
from .sse_views import _event_stream
...
class PubSubTests(SimpleTestCase):
    ...
    async def test_publish_from_a_worker_thread_reaches_the_loop(self):
        # The real callers (DRF views, the admin) are sync, so under ASGI
        # they publish from Django's thread executor, not the event loop.
        async with subscribe(TEAM_ID) as events:
...
class EventStreamTests(SimpleTestCase):
    """The SSE endpoint itself. No database: the stream view authenticates
    from the in-memory ticket store and never queries.
    """
    ...
    async def test_stream_delivers_events_for_the_ticketed_team(self):
        ...
        response = await client.get(f"/api/teams/stream/?ticket={ticket}")
        ...
        self.assertEqual(await _read(body), b": connected\n\n")

        publish(TEAM_ID, "team_attacked", {"life": 2, "damage": 1})

        self.assertEqual(
            await _read(body),
            b'event: team_attacked\ndata: {"life": 2, "damage": 1}\n\n',
        )

    async def test_stream_rejects_a_missing_or_spent_ticket(self):
        client = AsyncClient()
        ticket = issue_ticket(TEAM_ID)
        consume_ticket(ticket)
        ...
        for query in ("", f"?ticket={ticket}", "?ticket=bogus"):
            with self.subTest(query=query):
                response = await client.get(f"/api/teams/stream/{query}")

                self.assertEqual(response.status_code, 401)

    async def test_client_disconnect_unsubscribes(self):
        # Closing the response generator is exactly what Django's ASGI
        # handler does (via aclosing) when the browser goes away.
...
class StreamTicketEndpointTests(APITestCase):
    def setUp(self):
    ...
    def test_issues_a_ticket_for_the_callers_team(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            consume_ticket(response.data["ticket"]), self.user.team_id
        )
